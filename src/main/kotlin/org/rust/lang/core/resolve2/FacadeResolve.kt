...
interface DataPsiHelper {
    fun psiToData(mod: RsMod): ModData?
    fun dataToPsi(modData: ModData): RsMod?
}
...
private fun Project.getDetachedModInfo(scope: RsMod): RsModInfoBase {
    val rootMod = scope.containingFile as? RsFile ?: return InfoNotFound
    val crate = FakeDetachedCrate(rootMod, dependencies = emptyList())
...
private fun Project.getModInfoInDetachedCrate(scope: RsMod, crate: Crate): RsModInfoBase {
    val defMap = defMapService.getDetachedDefMap(crate) ?: return InfoNotFound
    val dataPsiHelper = DetachedFileDataPsiHelper(crate.rootMod ?: return InfoNotFound, defMap)
    val modData = dataPsiHelper.psiToData(scope) ?: return InfoNotFound
...
private class DetachedFileDataPsiHelper(
    ...
    override fun psiToData(mod: RsMod): ModData? {
        return when {
            mod.containingFile != root -> null
            mod == root -> defMap.root
            mod is RsModItem -> {
                val superMod = mod.`super` ?: return null
                val superModData = psiToData(superMod) ?: return null
                superModData.childModules[mod.modName]
            }
            else -> null
        }
    }

    override fun dataToPsi(modData: ModData): RsMod? {
        return when {
            modData.crate != defMap.crate -> null
            modData == defMap.root -> root
            else -> {
                val superModData = modData.parent ?: return null
                val superMod = dataToPsi(superModData) ?: return null
                superMod.getChildModule(modData.name)
            }
        }
    }
}
